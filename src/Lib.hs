...
runManager m_data =
  do execute 0
     return ()
  where execute count =
          do putStrLn $ "Checking for new traces: " ++ show count
             new_traces <- purgeTQueue $ mdTraceQueue m_data
             let added = fromIntegral (length new_traces)
             putStrLn $ "new traces count: " ++ show added
             let n = min added (mdNumTraces m_data - count) :: Integer
                 new_count = n + count
             if n > 0
               then do putStrLn $ "Writing " ++ show n ++ " traces"
                       mapM_ (writeToFile (mdOutHandle m_data)) (map encode (take (fromIntegral n) new_traces))
                       putStrLn "Done"
               else return ()
             putStrLn ""
             unless (new_count >= mdNumTraces m_data) $
               do putStrLn "sleeping for a while"
                  threadDelay (1000 * 1000) -- Sleep 1s
                  putStrLn "woke up"
                  execute new_count

-- | The actions to be performed by a worker thread.
runWorker :: WorkerData -> IO ()
runWorker w_data = loop 0 0
  where
  loop n k =
    do c <- randomCoord
       n' <- 
...
main =
  do t_queue <- newTQueueIO
     ...
     putStrLn $ "number of workers: " ++ show (length workers)
     runManager m_data
